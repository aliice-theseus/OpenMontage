...
HEYGEN_PROVIDERS = {
    ...
    # access today, use `seedance_video` or `seedance_replicate`.
...
def upload_image_heygen(image_path: str, api_key: str) -> str:
    ...
    if resp.status_code != 200:
        raise RuntimeError(f"HeyGen upload failed: {resp.status_code} {resp.text}")

    data = resp.json().get("data", {})
    upload_url = data.get("upload_url")
    file_url = data.get("url") or data.get("file_url")
    if not upload_url or not file_url:
        raise RuntimeError(f"HeyGen upload response missing upload_url or url: {data}")
