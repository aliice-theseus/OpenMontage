...
针对所有管道，只要下一阶段会生成 AI 视频片段、数字人/口型视频、角色动画视频或最终合成视频，就必须在进入该阶段前获得用户的明确确认动作。该规则由清单字段 `entry_human_approval_required: true` 声明，并由运行时门禁强制执行。

- 在确认请求中说明确切工具、提供商/模型、样品或批量模式、预计成本与输出。
- 用户此前批准 proposal、script、scene_plan 或其他阶段，不等于批准当前视频生成动作。
- 沉默、超时、会话恢复和推断同意都不算确认。
- 每个受保护阶段、每个新批次以及任何重大生成路径变更都必须重新确认。
- 收到确认后，仅对当前阶段设置 `PipelineContext.entry_human_approved=True`；不得跨阶段复用。
- 未确认时必须停下等待，禁止清除流水线上下文、直接调用提供商或以其他方式绕过门禁。
