---
name: photo-avatars
description: 从照片创建虚拟角色（说话照片）用于 HeyGen
---

# 照片虚拟角色（说话照片）

照片虚拟角色允许您将静态照片动画化并使其说话。这对于从肖像、头像或任何合适的图像创建个性化视频内容非常有用。
...
使用资产上传端点上载肖像照片。响应中包含一个 `image_key`，您将在下一步中使用。
...
详见 [assets.md](assets.md) 了解完整的上传详情。

### 步骤 2：创建照片虚拟角色组

使用上传响应中的 `image_key` 创建照片虚拟角色组。这将处理图像并创建一个可用的照片虚拟角色。
...
等待 `status` 变为 `"completed"` 后再用于视频生成。

### 步骤 4：在视频生成中使用

使用照片虚拟角色的 `id` 作为 `talking_photo_id`：
...
每个 `id` 可以在视频生成中用作 `talking_photo_id`。
...
Avatar IV 是 HeyGen 最新的照片虚拟角色技术，具有改进的质量和自然动作。它直接从上传的图像生成视频，跳过了虚拟角色组创建步骤。
...
| 字段 | 类型 | 必需 | 描述 |
|-------|------|:---:|-------------|
| `image_key` | string | ✓ | 资产上传中的 S3 图像键 |
| `script` | string | ✓ | 虚拟角色要说的文本 |
...
从文本描述生成合成照片虚拟角色，而不是上传照片。

**端点：** `POST https://api.heygen.com/v2/photo_avatar/photo/generate`

> **重要：所有 8 个字段都是必需的。** API 将拒绝缺少任何字段的请求。
> 当用户要求"生成一个专业男士的 AI 虚拟角色"时，您需要询问或选择所有以下字段的值。

### 必需字段（必须全部提供）

| 字段 | 类型 | 允许值 |
|-------|------|----------------|
| `name` | string | 生成虚拟角色的名称 |
...
在调用 AI 生成 API 之前，确保您拥有所有字段的值：

| # | 字段 | 要问的问题 / 默认值 |
|---|-------|---------------------------|
| 1 | `name` | 我们应该如何命名这个虚拟角色？ |
| 2 | `age` | 年轻成人 / 早期中年 / 晚期中年 / 老年？ |
| 3 | `gender` | 女士 / 男士？ |
| 4 | `ethnicity` | 哪个族裔？（见上方枚举值） |
| 5 | `orientation` | 水平（横屏）/ 垂直（竖屏）/ 方形？ |
| 6 | `pose` | 半身（推荐）/ 特写 / 全身？ |
| 7 | `style` | 真实（推荐）/ 电影感 / 其他？ |
| 8 | `appearance` | 描述服装、表情、灯光、背景 |

**如果用户只提供模糊请求**，如"创建一个看起来专业的男士"，请要求他们指定缺失的字段或做出合理的默认值（例如"早期中年"、"真实"风格、"半身"姿势、"水平"方向）。

### 外观提示技巧

`appearance` 字段是一个文本提示——要描述性：
...
| 端点 | 方法 | 描述 |
|----------|--------|-------------|
| `upload.heygen.com/v1/asset` | POST | 上传图像（返回 `image_key`） |
...
5. **训练虚拟角色组** - 提高动画质量
6. **复用照片虚拟角色 ID** - 创建后，在多个视频中使用相同的 `talking_photo_id`
