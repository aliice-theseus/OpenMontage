...
如果 HeyGen MCP 服务器已连接，使用 `mcp__heygen__get_video` 并传入 `videoId` 参数。它会在一次调用中返回状态、video_url、thumbnail_url、duration、title、gif_url、captioned_video_url 和其他元数据。
...
视频完成后，进行下载。**重要提示**：状态显示"completed"后，视频 URL 可能不会立即可用。使用带退避的重试逻辑。
...
对于长时间运行的生成，保存 video_id 并稍后检查状态，而不是保持进程等待。
...
您可以不使用轮询，而是使用 webhook 在视频完成时接收通知。详见 [webhooks.md](webhooks.md)。对于不想维持轮询连接的生产系统，Webhook 是理想选择。

## 最佳实践

1. **使用指数退避** - 为长时间运行的任务增加轮询间隔
2. **设置合理的超时时间** - 大多数视频在 10 分钟内完成
3. **优雅地处理失败** - 检查错误消息以获取可操作的反馈
