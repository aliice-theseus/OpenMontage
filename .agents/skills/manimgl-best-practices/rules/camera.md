...
ManimGL 的相机系统以 `CameraFrame` 为中心，通过 `self.camera.frame` 访问。这提供了对 2D 和 3D 视角的强大控制。
...
`reorient()` 是在 ManimGL 中设置 3D 相机朝向的主要方法。
...
# 常见视角：
frame.reorient(0, 0)        # 前视图（XY平面）
frame.reorient(20, 70)      # 等轴测类视图
frame.reorient(0, 90)       # 俯视图（从上方看XY平面）
frame.reorient(90, 90)      # 侧视图（YZ平面）
...
1. **存储帧引用**：在开始时使用 `frame = self.camera.frame`
2. **3D 使用 reorient()**：比单独设置角度更简洁
3. **UI 使用 fix_in_frame()**：保持标签和标题可读
4. **平滑过渡**：为相机移动使用适当的 run_time
5. **rate_func=linear**：用于连续旋转
6. **to_default_state()**：需要时重置相机
7. **跟随使用 Updater**：使用 updater 跟踪移动对象
