...
**重要提示：** ManimGL 使用 `Tex` 类（不是 ManimCE 中的 `MathTex`）。

```python
# ManimGL - 使用 Tex 和大写 R 原始字符串
...
# 现在 formula[0] 是 "a^2"，formula[1] 是 "+"，以此类推
```
...
1. 检查 LaTeX 安装中是否缺少宏包
2. 先尝试更简单的表达式
3. 检查输出目录中的中间 `.tex` 文件
4. 在数学模式中使用 `\text{}` 处理常规文本
