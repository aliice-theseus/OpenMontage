...
ManimGL 为 mobject 提供了全面的样式选项，包括填充、描边、不透明度和特殊效果。
...
`backstroke` 功能在文本或图形后方添加轮廓以提高可见性。
...
sphere.set_gloss(0.8)  # 0（哑光）到1（非常亮）
...
# 为3D对象添加阴影
cube = Cube(color=RED)
cube.set_shadow(0.6)  # 0（无阴影）到1（强阴影）
...
1. **透明度用于分层**：使用透明度展示重叠元素
2. **背光描边提升可读性**：在复杂背景上为文本添加背光描边
3. **一致的描边宽度**：保持一致的宽度以维持视觉层次
4. **填充 vs 描边**：区域使用填充，边框使用描边
5. **光泽增加真实感**：为3D对象添加光泽使外观更真实
6. **样式匹配保证一致性**：使用样式匹配实现一致的外观
7. **渐变表示流动**：使用渐变展示过渡或关系
