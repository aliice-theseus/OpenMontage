---
name: ffmpeg
description: 使用 FFmpeg 进行视频和音频处理。用于格式转换、调整大小、压缩、音频提取和为 Remotion 准备素材。触发条件包括将 GIF 转换为 MP4、调整视频大小、提取音频、压缩文件或任何媒体转换任务。
---
...
**为什么使用这些标志：**
- `-movflags faststart` - 将元数据移到开头以实现网络流式传输
- `-pix_fmt yuv420p` - 确保与大多数播放器的兼容性
- `scale=trunc(...)` - 强制偶数尺寸（大多数编解码器需要）

### 调整视频大小

```bash
# 到 1920x1080（保持宽高比，添加黑边）
...
**注意：** 建议重新编码进行修剪。如果定位点不与关键帧对齐，流复制（`-c copy`）可能静默丢弃视频。
...
| 变速（随时间加速） | ✅ 预处理 | 需要复杂变通 |
| 需要完美音频同步 | ✅ 有保证 | 通常没问题 |
| 演示需要匹配画外音时间 | ✅ 预先计算 | 运行时调整 |

**Remotion 限制：** `playbackRate` 必须是常数。像 `playbackRate={interpolate(frame, [0, 100], [1, 5])}` 这样的动态插值无法正常工作，因为 Remotion 独立评估帧。

```bash
# 加速演示以适配场景（例如 60s 演示到 20s = 3 倍速）
...
- setpts 倍率 = `1 / speed`（例如 3 倍速 = setpts=0.333*PTS）
- atempo 值 = `speed`（例如 3 倍速 = atempo=3.0）

**极端速度（>2x 音频）：** 链式 atempo 滤镜（每个限制在 0.5-2.0 范围）：
```bash
# 4 倍速音频
...
```bash
# 从 iPhone/iPad 录制（通常是 60fps，可变分辨率）
...
Remotion 渲染视频（通常到 `out/video.mp4`）后，使用 FFmpeg 为每个分发平台优化。
...
Twitter 有严格限制：最大 140 秒、512MB、1920x1200：
