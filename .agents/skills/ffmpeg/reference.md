...
| libx265 | H.265/HEVC | 更好压缩，兼容性较低 |
| libvpx-vp9 | WebM | 适合网络 |
| prores | ProRes | 专业编辑 |
| copy | 流复制 | 无需重新编码，最快 |
...
| -y | 覆盖输出 | `-y` |
| -n | 永不覆盖 | `-n` |
| -movflags faststart | 网络流式传输 | `-movflags faststart` |
...
- Remotion 使用 `<OffthreadVideo>` 处理大多数格式
- 偏好 MP4 容器中的 H.264（libx264）
- 始终使用 `-movflags faststart` 进行网络播放
- 匹配 fps 到合成（通常 30fps）
- 分辨率应匹配合成（通常 1920x1080）
