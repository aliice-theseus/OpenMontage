...
此技能需要 `agent-browser` 技能进行 PNG 渲染。在继续 PNG 捕获之前先加载它。
...
如果用户提供的是描述而非代码，生成有效的 Mermaid 语法。完整语法详情请查阅 `references/mermaid-syntax.md`。
...
这会创建一个最小的 HTML 文件，显示带有适当内边距和背景的 SVG。

### 步骤 4：使用 agent-browser 捕获高分辨率 PNG

使用 agent-browser CLI 捕获高质量截图。完整 CLI 文档请参阅 `agent-browser` 技能。
...
对于复杂图表需要更高分辨率，进一步增加视口或在创建 HTML 包装器时使用 `--padding` 选项为图表提供更多空间。

### 步骤 5：清理中间文件

渲染完成后，移除所有中间文件。只应保留最终的 `.svg` 和 `.png`。
...
如果创建了临时的 `.mmd` 文件，也请一并移除。

## 输出

总是生成两种输出：
- **SVG**：矢量格式，无限可缩放，文件大小小
- **PNG**：高分辨率光栅图，以 4K（3840×2160）视口捕获，图表宽度最小 1200px

文件保存到当前工作目录，除非用户明确指定不同路径。
...
| default | 浅灰 | 通用 |
| dracula | 深紫 | 偏好深色模式 |
| tokyo-night | 深蓝 | 现代深色美学 |
| tokyo-night-storm | 深蓝 | 更高对比度 |
| nord | 深北极 | 柔和、平静的视觉效果 |
| nord-light | 浅北极 | 带柔和色调的浅色模式 |
| github-dark | GitHub 深色 | 匹配 GitHub UI |
| github-light | GitHub 浅色 | 匹配 GitHub UI |
| catppuccin-latte | 暖浅 | 柔和粉彩美学 |
| solarized | 棕褐色 | Solarized 颜色方案 |
| one-dark | Atom 深色 | Atom 编辑器美学 |
| zinc-dark | 中性深色 | 极简，无色偏 |

## 故障排除

### 主题未应用

检查渲染脚本输出中的 `bg` 和 `fg` 值，或检查 SVG 的起始标签中的 `--bg` 和 `--fg` CSS 自定义属性。

### 图表显示为截断或不完整

- 检查边标签语法——使用 `-->|label|` 管道符号，不要用 `-- label -->`
- 验证所有节点 ID 是否唯一
- 检查节点标签中是否有未闭合的括号

### 渲染生成空或格式错误的 SVG

- 在 https://mermaid.live 验证 Mermaid 语法后再渲染
- 检查需要转义的特殊字符（用引号包裹）
