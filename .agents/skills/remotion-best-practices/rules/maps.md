...
[Mapbox 文档](https://docs.mapbox.com/mapbox-gl-js/api/) 包含 API 参考。
...
用户需要创建一个免费的 Mapbox 账户并访问 https://console.mapbox.com/account/access-tokens/ 创建访问令牌。
...
export const MyComposition = () => {
  ...
  const [handle] = useState(() => delayRender("正在加载地图..."));
...
以下是在 Remotion 中需要注意的要点：

- 动画必须由 `useCurrentFrame()` 驱动，并且 Mapbox 自带的动画应被禁用。例如，`fadeDuration` 属性应设置为 `0`，`interactive` 应设置为 `false` 等。
- 应使用 `useDelayRender()` 延迟地图加载，并且在地图加载完成前将地图设置为 `null`。
- 包含 ref 的元素必须有明确的宽度、高度和 `position: "absolute"`。
- 不要添加 `_map.remove();` 清理函数。

## 绘制线条

除非我要求，否则不要在线条上添加发光效果。
除非我要求，否则不要在线条上添加额外的点。
...
除非我另有要求，否则移除 Mapbox Standard 样式的所有功能。
...
你可以通过添加一个 `useEffect` 钩子来沿线路制作摄像机动画，该钩子根据当前帧更新摄像机位置。

除非我要求，否则不要在摄像机角度之间跳转。
...
useEffect(() => {
  ...
  const handle = delayRender("正在移动点...");
...
重要：默认保持摄像机北方朝上。
重要：对于多步骤动画，在所有阶段设置所有属性（缩放、位置、线条进度）以防止跳跃。覆盖初始值。

- 进度被限制在最小值，以避免线条为空，这可能导致 turf 错误
- 有关计时的更多选项，请参阅[计时](./timing.md)
...
要制作在地图上显示为直线的动画，请在坐标之间使用线性插值。不要使用 turf 的 `lineSliceAlong` 或 `along` 函数，因为它们使用测地线（大圆）计算，在墨卡托投影上会显示为曲线。
...
useEffect(() => {
  ...
  const animationHandle = delayRender("正在绘制线条...");
...
要制作遵循两点之间测地线（大圆）路径的线条动画，请使用 turf 的 `lineSliceAlong`。这对于显示飞行路径或地球上的实际最短距离非常有用。
...
确保它们足够大。检查合成的尺寸并相应缩放标签。
对于 1920x1080 的合成尺寸，标签字体大小应至少为 40px。

重要：保持 `text-offset` 足够小，使其靠近标记。考虑标记圆半径。对于半径为 40 的圆，这是一个合适的偏移量：
