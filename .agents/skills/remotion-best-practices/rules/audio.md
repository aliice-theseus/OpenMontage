...
使用 `@remotion/media` 的 `<Audio>` 组件将音频添加到合成中。
...
使用 `toneFrequency` 来调整音调而不影响速度。取值范围为 0.01 到 2：
