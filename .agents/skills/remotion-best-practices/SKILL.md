...
当处理 Remotion 代码时，使用此技能以获取领域特定知识。

## 字幕

处理字幕（captions/subtitles）时，加载 [./rules/subtitles.md](./rules/subtitles.md) 文件获取更多信息。
...
- [rules/calculate-metadata.md](rules/calculate-metadata.md) - 动态设置合成的时长、尺寸和属性
...
- [rules/extract-frames.md](rules/extract-frames.md) - 使用 Mediabunny 在指定时间戳从视频中提取帧
...
- [rules/measuring-text.md](rules/measuring-text.md) - 测量文本尺寸、将文本适配到容器中以及检查溢出
- [rules/sequencing.md](rules/sequencing.md) - Remotion 的序列编排模式 - 延迟、裁剪、限制项目时长
...
- [rules/timing.md](rules/timing.md) - Remotion 中的插值曲线 - 线性、缓动、弹性动画
...
- [rules/parameters.md](rules/parameters.md) - 通过添加 Zod 模式使视频参数化
