---
name: colors
description: Manim 中的颜色常量、渐变和颜色操作
metadata:
...
# 修改不透明度
circle.set_opacity(0.5)         # 同时影响填充和描边
...
1. **使用颜色变体表现深度** - `BLUE_E` 用于阴影，`BLUE_A` 用于高亮
2. **保持颜色一致** - 相关概念使用相同颜色
3. **使用不透明度进行分层** - 半透明填充显示重叠
4. **考虑色盲可访问性** - 避免仅使用红绿区分
5. **谨慎使用渐变** - 可能分散注意力
