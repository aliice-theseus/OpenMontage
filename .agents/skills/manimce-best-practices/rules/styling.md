---
name: styling
description: mobject 的填充、描边、不透明度和视觉样式
metadata:
...
mobject.set_fill_color(RED)                    # 仅颜色（替代）
mobject.set_fill_opacity(0.5)                  # 仅不透明度（替代）
```
...
mobject.set_stroke_color(RED)                  # 仅颜色（替代）
mobject.set_stroke_width(4)                    # 仅宽度（替代）
mobject.set_stroke_opacity(0.5)                # 仅不透明度（替代）
```
...
# 淡入效果
mobject.fade(0.5)  # 0.5 = 50% 淡出（与不透明度相反）
```

## 背景矩形

在文本或其他 mobject 后面添加背景：
...
1. **形状使用 fill_opacity** - 完全不透明的填充可能隐藏其他元素
2. **一致的描边宽度** - 选择一个宽度并保持一致
3. **填充和描边形成对比** - 不同颜色有助于定义
4. **使用 BackgroundRectangle 提升可读性** - 在复杂背景上放置在文本后方
5. **链式方法编写简洁代码** - 但如果太长，请换行
