...
Group 用于混合不同类型的 mobject（VMobject、ImageMobject 等）。
...
1. **VMobject 使用 VGroup** - 更好的性能和兼容性
2. **创建后立即 arrange** - 不要单独定位再分组
3. **语义化命名组** - 使用 `equation_parts` 而非 `group1`
4. **嵌套组实现结构** - 行列嵌套等
5. **需要时复制组** - 避免意外修改
