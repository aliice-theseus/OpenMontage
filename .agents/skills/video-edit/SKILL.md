...
name: video-edit
description: |
  使用 ffmpeg 在本地编辑视频。修剪、拼接、调整大小、变速、叠加、提取音频、压缩和转换格式。
  在以下情况使用：(1) 修剪或剪切视频片段，(2) 拼接多个剪辑，(3) 为社交平台调整视频大小，
...
配合使用：`ffmpeg -y -i input.mp4 -vf "<filter>" -c:a copy output.mp4`

## 提示

- 始终使用 `-y` 覆盖输出而不提示。
- 仅需剪切/连接时使用 `-c copy`（无需重新编码，非常快）。
- CRF 越低 = 质量越好、文件越大。范围 18-28 为典型值；23 为默认值。
- 详细配方和标志说明请参阅 `references/operations.md`。
