...
本指南介绍生成 HeyGen 虚拟角色视频并在 Remotion 合成中使用的工作流。
...
| 您的合成 | 推荐 | 原因 |
|------------------|-------------|-----|
| 虚拟角色作为主持人带覆盖层 | MP4 + 背景 | 更简单，覆盖层在顶部 |
| Loom 风格（虚拟角色叠加在屏幕录制上） | WebM + `closeUp`，在 Remotion 中遮罩 | 需要透明度，在 CSS 中应用圆形遮罩 |
| 虚拟角色叠加在_其他_视频/内容上 | WebM（透明）| 需要透视看到背后的内容 |
| 全屏虚拟角色 | MP4 + 背景 | 标准方法 |

**大多数情况下使用带背景的 MP4。** 当您需要看到虚拟角色_背后_的内容时使用 WebM。

**注意：** WebM 仅支持 `normal` 和 `closeUp` 风格。对于圆形构图，在 Remotion 中使用 CSS `border-radius: 50%`。

## 推荐：并行开发工作流

HeyGen 视频生成需要 **10-15 分钟以上**。不要等待 — 并行工作：

1. **开始 HeyGen 生成** — 保存 `video_id` 到文件，立即退出
2. **构建 Remotion 合成** — 使用占位符或虚拟角色的 `preview_video_url`（短视频循环）
3. **定期检查 HeyGen 状态**，或在构建完成后检查
4. **将占位符替换**为就绪后的真实视频 URL

**从脚本估计时长**：~150 单词/分钟语速，所以 `wordCount / 150 * 60 * fps` 给出大致帧数。

**合成技巧**：设计组件使其无论有无虚拟角色视频都能工作，这样动态图形可以独立测试。
...
大多数 Remotion 合成最适合使用 MP4 + 背景。覆盖层和动态图形放在上方：
...
仅当您需要看到虚拟角色_背后_的内容时使用（例如虚拟角色叠加在屏幕录制上）：
...
const response = await fetch("https://api.heygen.com/v1/video.webm", {
  ...
  body: JSON.stringify({
    avatar_pose_id: avatarPoseId,  // 必填：虚拟角色姿势 ID
    avatar_style: "normal",        // 必填：仅 "normal" 或 "closeUp"
...
### 重要：使用 OffthreadVideo 获得帧精确渲染

**始终使用 `OffthreadVideo` 而非 `Video`** 来处理 HeyGen 虚拟角色视频。基本的 `Video` 组件使用浏览器的视频解码器，不是帧精确的，会导致渲染时出现抖动。`OffthreadVideo` 通过 FFmpeg 提取帧，实现流畅、精确的播放。
...
export const AvatarWithMotionGraphics: React.FC<{
  ...
  return (
    <AbsoluteFill>
      ...
      {/* 层 2：带透明背景的虚拟角色 - 使用 OffthreadVideo 获得帧精确渲染 */}
      <OffthreadVideo
        src={avatarWebmUrl}
        transparent
        style={{
...
**注意：** WebM 不支持 `circle` 风格 — 使用 `normal` 或 `closeUp` 并通过 CSS 应用圆形遮罩。
...
export const LayeredAvatarComposition: React.FC<LayeredAvatarProps> = ({
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {/* 层 2：虚拟角色视频 - 使用 OffthreadVideo 防止抖动 */}
      <OffthreadVideo
        src={avatarVideoUrl}
        style={{
          position: "absolute",
...
1. **使用 `OffthreadVideo` 替代 `Video`** — 基本的 `Video` 组件使用浏览器的视频解码器，不是帧精确的
