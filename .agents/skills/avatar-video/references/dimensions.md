...
HeyGen 支持各种视频尺寸和宽高比，以适应不同平台和使用场景。
...
const youtubeConfig = {
  video_inputs: [...],
  dimension: { width: 1920, height: 1080 }, // 16:9 横向
};
```
...
const shortFormConfig = {
  video_inputs: [...],
  dimension: { width: 1080, height: 1920 }, // 9:16 竖向
};
```
...
const linkedinConfig = {
  video_inputs: [...],
  dimension: { width: 1920, height: 1080 }, // 推荐 16:9 横向
};
```
...
const twitterConfig = {
  video_inputs: [...],
  dimension: { width: 1280, height: 720 }, // 16:9，720p 常见
};
```

## Avatar IV 尺寸

对于 Avatar IV（基于照片的虚拟角色），通过方向设置尺寸：
...
考虑在草稿和测试中使用 720p，然后在最终输出中使用 1080p。
