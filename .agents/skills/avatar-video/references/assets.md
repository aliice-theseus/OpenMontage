...
HeyGen 允许您上传自定义资产（图片、视频、音频），用于视频生成，如背景、会说话的照片源和自定义音频。

## 上传流程

资产上传是一个单步过程：直接将原始文件二进制 POST 到上传端点。Content-Type 头部必须匹配文件的 MIME 类型。
...
| `data.image_key` | string \| null | 用于创建上传照片虚拟角色的键（仅图片）|
...
- **图片尺寸**：建议匹配视频尺寸
- **音频时长**：应与预期视频长度匹配
- **保留期**：资产可能在一段时间不活动后被删除

## 最佳实践

1. **优化图片** — 在上传前调整大小以匹配视频尺寸
