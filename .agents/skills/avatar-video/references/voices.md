---
name: voices
description: 列出 HeyGen 的声音、区域设置、语速/音调配置
---

# HeyGen 声音

HeyGen 为不同的语言、口音和风格提供了各种各样的 AI 声音。声音将您的文字脚本转换为自然流畅的语音。
...
HeyGen 支持 SSML 风格的 `<break>` 标签，在脚本中添加暂停。
...
其中 `X` 是以秒为单位的持续时间（例如 `1s`、`1.5s`、`0.5s`）。

### 要求

| 规则 | 示例 |
|------|---------|
| 使用带 "s" 后缀的秒 | `<break time="1.5s"/>` ✓ |
...
1. **用于强调** — 在重要点之前添加暂停
2. **保持暂停合理** — 0.5 秒到 2 秒是典型的；更长会显得不自然
3. **匹配自然语言** — 在人类会呼吸或停顿的地方添加暂停
4. **测试输出** — 听生成的音频以验证节奏感觉合适
...
1. **匹配声音性别与虚拟角色** — 始终将男性声音与男性虚拟角色配对，女性与女性配对
2. **匹配声音与内容** — 商务内容使用专业声音
3. **测试声音预览** — 在选择前听取预览音频
4. **考虑区域设置** — 匹配声音口音与目标受众
5. **使用自然节奏** — 为清晰度调整语速，通常 0.9-1.1 倍
6. **添加暂停** — 使用 SSML 断点获得更自然的语言流
