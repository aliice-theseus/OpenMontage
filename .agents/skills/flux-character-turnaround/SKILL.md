...
生成同一人物、同一服装和统一比例的建模参考图。虽然用户常称“四视图”，本技能的标准交付物是三个全身视角加一个胸部以上特写。
...
4. 使用分步流程：正面全身 T2I → 以正面图为共同参考分别生成侧面、背面和胸部以上特写 → 本地确定性拼接。不要要求扩散模型一次生成四栏，因为这会显著增加身份漂移、布局错位和肢体裁切。
...
- 最终横向四栏由本地拼接保证：正面 281 px、侧面 281 px、背面 281 px、胸部以上特写 437 px，总计 `1280 × 720`，栏间距 0 px。
- 纯白无缝背景，无场景、道具、装饰、文字或水印。
- 四栏必须是同一人物、同一服装、同一发型、同一配色、同一人体比例和同一棚拍光。
- 三个全身视图必须从头到脚完整可见，标准自然站姿，四肢无遮挡。
- 正面胸部以上特写必须包含头部到胸部（含完整胸线），清楚展示五官、发型、领口、锁骨、肩线、胸型与配饰位置。

正面胸部以上特写图拼接前保留顶部 75%，确保构图下缘在胸线附近且不裸露腰部。各栏使用 LANCZOS 缩放和居中裁切。分栏布局由代码保证，但人物是否完整、视角是否正确仍须目视检查。
