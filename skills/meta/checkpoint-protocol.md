...
`human_approval_default` 是阶段完成后的审批，不能作为开始生成视频的授权。任何清单中设置了
`entry_human_approval_required: true` 的阶段，在执行该阶段任何工具前必须：

1. 向用户列明本阶段将执行的视频生成动作、确切工具与提供商/模型、样品或批量模式、预计成本和预期输出。
2. 等待用户作出明确确认动作。沉默、未回复、此前对 proposal/script/scene_plan 的批准，以及恢复旧会话，都不算本阶段确认。
3. 仅在收到确认后，以 `entry_human_approved=True` 创建当前阶段的 `PipelineContext` 并执行。
4. 每进入一个新的受保护阶段都重新确认；确认不得跨阶段、跨批次或跨重大生成方案复用。

未确认时，运行时会抛出 `StageEntryApprovalRequiredError`。此时应停下并请求确认，不得绕过流水线上下文或直接调用提供商工具。
...
6. **视频生成必须先确认。** 所有流程只要进入会生成移动影像或最终视频文件的阶段，就必须执行阶段入口确认；上游创意审批不能替代这个动作。
