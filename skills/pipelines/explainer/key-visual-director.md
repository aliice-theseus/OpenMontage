...
1. 为视频中的**每个角色**生成**三视图概念设计**（正面/侧面/背面）
2. 为**每个场景**生成**关键帧构图**
3. 将关键图展示给用户并获得**明确确认**
4. 确认后才能进入合成阶段
...
| 问题 | 后果 | 解决方案 |
|------|------|---------|
| 角色外观未定义 | Seedance 视频生成中角色漂移/变形 | 三视图锁定角色身份 |
...
从 `scene_plan` 中提取角色列表。每个角色需要三视图概念设计。
...
- 提示词中指定：`三视图概念设计,角色设计稿,统一风格,白底,全身完整`
