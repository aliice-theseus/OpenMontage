...
| `eleven_turbo_v2_5` | 快速、良好质量 | 32 | 低 |
...
| `similarity_boost` | 0.0-1.0 | 0.75 | 越高越接近原始语音 |
| `style` | 0.0-1.0 | 0.0 | 风格夸张程度（v2 模型） |
...
| 格式代码 | 采样率 | 比特率 | 所需套餐 |
|-------------|-------------|---------|---------------|
| `mp3_44100_128` | 44.1kHz | 128kbps | Free（默认） |
| `mp3_44100_192` | 44.1kHz | 192kbps | Creator+ |
| `pcm_44100` | 44.1kHz | - | Pro+ |
| `ulaw_8000` | 8kHz | - | Free（电话质量） |
...
需要 Creator 以上计划。创建微调模型（3-6 小时训练）。
...
|-----------|------|----------|-------------|
| `text` | string | 是 | 音效描述 |
| `duration_seconds` | float | 否 | 1-22 秒（省略则自动） |
| `prompt_influence` | float | 否 | 0.0-1.0（默认 0.3） |

**计费：** 100 字符/次（自动）或 25 字符/秒（固定时长）

**示例提示词：**
- 环境音："Rain on a tin roof, steady and rhythmic"
- 动作音："Sword being drawn from sheath, metallic ring"
- 机械音："Old car engine struggling to start then roaring to life"
...
| `composition_plan` | object | 是* | 详细作曲结构 |
| `duration_ms` | int | 否 | 10000-300000（10s-5min） |
| `instrumental` | bool | 否 | 强制纯器乐输出 |

*需要 `prompt` 或 `composition_plan` 其中之一，不能同时提供。

**有效的提示词包括：**
1. 流派/风格："indie rock"、"lo-fi hip hop"、"orchestral"
