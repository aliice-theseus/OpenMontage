...
<Video
  ...
  volume={0.5}                 // 0-1，或回调函数：(f) => f / 100
  playbackRate={1.5}
  muted={false}
  loop={false}
  startFrom={30}               // 跳过源的前 30 帧
  endAt={120}                  // 在源的 120 帧处停止
...
<OffthreadVideo
  src={staticFile('clip.mp4')}
  volume={0.5}
  transparent={false}          // 用于带 alpha 通道的视频
...
const props = getInputProps();  // 通过 --props CLI 标志传入的数据
```
