---
name: sound-effects
description: 使用 ElevenLabs 根据文本描述生成音效。适用于创建音效、生成音频纹理、制作环境音、电影冲击音、UI 音效或任何非语音的音频。支持循环、时长控制和提示词影响调整。
license: MIT
...
根据文本描述生成音效 — 支持循环、自定义时长和提示词遵循度控制。
...
| `duration_seconds` | number \| null | null（自动） | 时长 0.5–30s；为 null 时自动计算 |
...
将 `output_format` 作为查询参数（cURL）或 SDK 参数传入：
...
- 具体描述："Heavy rain on a tin roof" 优于 "Rain"
...
常见错误：
- **401**：API 密钥无效
- **422**：参数无效（检查时长范围、prompt_influence 范围）
- **429**：超过频率限制
