...
避免在样式写入和布局读取之间交错操作。当你在样式更改之间读取布局属性（如 `offsetWidth`、`getBoundingClientRect()` 或 `getComputedStyle()`）时，浏览器被迫触发同步回流。

**这样没问题（浏览器会批量处理样式更改）：**
```typescript
function updateElementStyles(element: HTMLElement) {
  // 每行都会使样式失效，但浏览器会批量重新计算
...
尽可能优先使用 CSS 类而非内联样式。CSS 文件由浏览器缓存，且类提供了更好的关注点分离，更易于维护。
