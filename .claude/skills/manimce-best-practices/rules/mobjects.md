---
name: mobjects
description: Mobject 类型、VMobject 和 Manim 中的 mobject 层级
metadata:
...
## VMobject（矢量化 Mobject）

你将使用的大多数形状都是 VMobject —— 它们由贝塞尔曲线定义。
...
mobject.get_top()         # 上边缘中心点
mobject.get_bottom()      # 下边缘中心点
mobject.get_left()        # 左边缘中心点
mobject.get_right()       # 右边缘中心点
```
...
Mobjects 可以包含其他 mobjects 作为子对象。
...
2. **优先使用 VGroup 而非 Group** —— VGroup 与大多数动画兼容更好
3. **重用对象时使用 copy()** —— 避免意外修改原始对象
4. **链式调用提高可读性** —— 但过长时请换行
