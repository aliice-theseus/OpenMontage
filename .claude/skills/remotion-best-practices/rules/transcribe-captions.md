...
要在 Remotion 中转录音频以生成字幕，可以使用 [`@remotion/install-whisper-cpp`](https://www.remotion.dev/docs/install-whisper-cpp) 包中的 [`transcribe()`](https://www.remotion.dev/docs/install-whisper-cpp/transcribe) 函数。
...
// 写入 public/ 文件夹，以便从 Remotion 获取
fs.writeFileSync("captions123.json", JSON.stringify(captions, null, 2));
```

单独转录每个片段，并创建多个 JSON 文件。

查看[显示字幕](display-captions.md)了解如何在 Remotion 中显示字幕。
