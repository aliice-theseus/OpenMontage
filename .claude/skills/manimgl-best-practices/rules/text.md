...
ManimGL 通过 `Text` 和 `TexText` 类提供强大的文本渲染能力。

## Text 类

`Text` 类使用系统字体渲染文本，提供丰富的样式选项。
...
`TexText` 将 LaTeX 渲染与文本结合，适用于混合文本和数学内容。
...
1. **字体可用性**：确保字体已安装在系统中
2. **UI 元素使用 Text**：对外观有更好的控制
3. **混合内容使用 TexText**：同时需要文本和数学时使用
4. **使用背景描边提高可见性**：在复杂背景上为文本添加背景描边
5. **使用 VGroup 进行布局**：将相关文本元素分组以便于定位
6. **t2c/t2f/t2s/t2w**：使用字典进行按单词的样式设置
