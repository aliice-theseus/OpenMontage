...
ManimGL 使用 `manimgl` 命令来渲染场景。它提供了强大的标志（flag）以适应不同的工作流程。
...
--skip_animations --embed   # 在结尾交互（跳过动画）
...
1. **开发时使用 -l**：低质量实现快速迭代
2. **调试时使用 -se**：在问题点进入交互模式
3. **快速检查使用 -s**：在完整渲染前验证最后帧
4. **最终使用 -h -o**：准备就绪时输出高质量
...
```bash
# 场景未找到
manimgl scene.py  # 如果不指定，会列出所有场景

# 找不到 manimlib
# 确保在 manimlib/ 所在目录或使用完整路径
