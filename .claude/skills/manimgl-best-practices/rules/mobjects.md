...
├── ImageMobject
├── Point
└── 3D 对象（Surface, ParametricSurface 等）
```
...
| 方法 | 描述 |
|--------|-------------|
| `.move_to(point)` | 将中心移动到指定点 |
| `.shift(vector)` | 按向量移动 |
| `.scale(factor)` | 按因子缩放 |
| `.rotate(angle)` | 按角度旋转（弧度） |
| `.next_to(mob, dir)` | 定位到另一个旁边 |
| `.align_to(mob, dir)` | 与另一个边缘对齐 |
| `.to_edge(dir)` | 移动到屏幕边缘 |
