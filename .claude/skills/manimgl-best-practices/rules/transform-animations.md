...
变换动画将一个 mobject 变形为另一个，或动画化 mobject 属性的变化。

## Transform

基本的 `Transform` 将一个 mobject 变为看起来像另一个。
...
# 按角度旋转
square = Square()
self.play(Rotate(square, PI / 2))  # 90 度
...
```python
circle = Circle()
self.play(ScaleInPlace(circle, 2))  # 加倍大小
...
5. **路径弧度**：
   - 添加 `path_arc=90*DEGREES` 实现弧形变换路径
